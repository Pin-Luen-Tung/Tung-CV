...
void boot() {
  ...
  //DMA len
  dma_addr_boot[0x100]= (unsigned int)(&_dram_i_end) - (unsigned int)(&_dram_i_start);
 ...
 *(&_imem_start+i) = dram_i_start[i];  
  ...
  */
  ...
  //DMA len
  dma_addr_boot[0x100]= (unsigned int)(&__data_end) - (unsigned int)(&__data_start);
  // Enable DMA Controller
  dma_addr_boot[0x40] = 1; // Enable DMA
  asm("wfi");
  // disable DMA Controller
 ...
 *(&__data_start+k) = data_paddr_start[k];
  ...
  //DMA len
  dma_addr_boot[0x100]= (unsigned int)(&__sdata_end) - (unsigned int)(&__sdata_start);
  // Enable DMA Controller
  dma_addr_boot[0x40] = 1; // Enable DMA
  asm("wfi");
  // disable DMA Controller
